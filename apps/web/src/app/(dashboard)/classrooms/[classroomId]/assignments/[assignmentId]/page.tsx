...
export default function AssignmentDetailPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const asgn = await apiFetch<Assignment>(`/assignments/${assignmentId}`);
      setAssignment(asgn);

      // Try fetching student's own submission
      ...
      // Try fetching all submissions (works if caller is teacher)
  ...
  if (loading || !assignment) {
    return <div className="p-8 text-gray-500">Loading assignment...</div>;
  }
    ...
    <div className="h-full flex flex-col overflow-y-auto p-8 bg-[#313338]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Teacher Grading Section (if submissions exist) */}
                  ...
                  <div key={sub.id} className="py-3 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-400">
                        Status: <span className="font-medium text-emerald-400">{sub.status}</span> • v{sub.version}
                      </div>
                    </div>
        ...
        {/* Right Column: Student Turn-In Card */}
                ...
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                  ...
                  {mySubmission.receipt && (
                    <div className="mt-2 pt-2 border-t border-emerald-500/20 text-[11px] font-mono text-emerald-300">
                      Receipt: {mySubmission.receipt.receiptCode}
                    </div>
                  )}
                </div>
              ...
              <form onSubmit={handleTurnIn} className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Enter submission notes, repository URL, or solution details..."
